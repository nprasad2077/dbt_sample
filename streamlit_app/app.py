...
@st.cache_data
def get_seasons():
    return query("select distinct season_start_year from main_intermediate.int_games_enriched order by season_start_year desc")
...
# ============================================================
    ...
    fig = px.bar(ratings, x="net_rtg", y="team", orientation="h",
                 ...
                 title=f"Net Rating by Team ({selected_season}-{str(selected_season+1)[2:]})")
    ...
    """)
    st.subheader("Shot Selection Style vs Winning")
    fig5 = px.bar(styles, x="shot_selection_style", y=["win_pct", "avg_off_rtg"],
                  barmode="group", title="Play Style Effectiveness")
    ...
    if selected_players:
        ...
        for _, row in comp.iterrows():
            fig4.add_trace(go.Scatterpolar(
                ...
                theta=["PPG", "APG", "RPG", "SPG", "BPG", "USG%", "TS%"],
                fill="toself", name=row["player_name"]
            ))
        fig4.update_layout(title="Player Radar Comparison", height=500)
        st.plotly_chart(fig4, use_container_width=True)
...
# ============================================================
    ...
    # Shot zone breakdown
    zone_data = query(f"""
        select sz.shot_distance_zone, sz.zone_group, sz.zone_category,
        ...
        group by sz.shot_distance_zone, sz.zone_group, sz.zone_category
